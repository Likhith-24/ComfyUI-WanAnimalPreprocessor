...
class AAPoseMeta:
    ...
    def from_kps_body(kps_body: np.ndarray, height: int, width: int):
        """Create pose metadata from animal body keypoints"""
    ...
    def from_humanapi_meta(meta):
        """Load animal pose metadata. Works with AP10k format (17 keypoints)"""
        pose_meta = AAPoseMeta()
        width, height = meta["width"], meta["height"]
        pose_meta.width = width
        pose_meta.height = height
        kps = np.array(meta["keypoints_body"], dtype=np.float32)
        pose_meta.kps_body = kps[:, :2] * (width, height)
        pose_meta.kps_body_p = kps[:, 2]
        return pose_meta

    @staticmethod
    def from_animalpose(animal_pose_det_res, height, width, dataset='ap10k'):
        """Load animal pose from detection results (AP10k or APT36k format)
        
        Args:
            animal_pose_det_res (dict): Animal pose detection result with structure:
                {
                    "bodies": {
                        "candidate": np.ndarray,  # keypoints (N, 17, 2) or (N, 17, 3)
                        "score": np.ndarray        # confidence scores
                    }
                }
            height (int): Image height
            width (int): Image width
            dataset (str): 'ap10k' or 'apt36k' (both have same 17 keypoint format)
            
        Returns:
            AAPoseMeta: Animal pose metadata
        """
        pose_meta = AAPoseMeta()
        
        # Extract body keypoints from detection result
        if "bodies" in animal_pose_det_res:
            bodies = animal_pose_det_res["bodies"]
            candidates = bodies.get("candidate", None)
            scores = bodies.get("score", None)
            
            if candidates is not None:
                # Check if candidates array is empty
                if candidates.size == 0:
                    # Return empty pose metadata
                    pose_meta.height = height
                    pose_meta.width = width
                    return pose_meta
                ...
                if candidates.shape[-1] == 3:
                    pose_meta.kps_body = candidates[:, :2].copy()
                    pose_meta.kps_body_p = candidates[:, 2].copy()
                elif candidates.shape[-1] == 2:
                    pose_meta.kps_body = candidates.copy()
                    pose_meta.kps_body_p = scores.copy() if scores is not None else np.ones(len(candidates))
                else:
                    raise ValueError(f"Unexpected keypoint shape: {candidates.shape}")
    ...
    def load_from_meta(self, meta, norm_body=True):
        """Load animal pose data from metadata dictionary"""
    ...
    @staticmethod
    def load_from_kp2ds(kp2ds: np.ndarray, width: int, height: int):
        """Load animal keypoints from AP10k format (17 keypoints, no hands/face)
        
        Args:
            kp2ds (np.ndarray): Keypoints array of shape (17, 3) for AP10k format
            width (int): Image width
            height (int): Image height
        ...
        pose_meta.width = width
        pose_meta.height = height
        # AP10k format: 17 keypoints total
        pose_meta.kps_body = kp2ds[:, :2]
        pose_meta.kps_body_p = kp2ds[:, 2]
        return pose_meta
    ...
    def padding_resize2(self, height=512, width=512):
        """Resize animal pose with padding. kps will be changed inplace"""
...
def split_kp2ds_for_animal(kp2ds):
    """Split animal keypoints (AP10k/APT36k 17-keypoint format).
    
    Animal keypoints are already in the correct order:
    ...
    Args:
        kp2ds: (17, 3) array of [x, y, confidence]
    
    Returns:
        kp2ds_body: (17, 3) body keypoints (all keypoints for animals)
    """
    return kp2ds.copy()
...
def split_kp2ds_for_aa(kp2ds, ret_face=False):
    """Backward compatibility wrapper - redirects to animal keypoint format."""
    kp2ds_body = split_kp2ds_for_animal(kp2ds)
    if ret_face:
        # No hands/face for animals - return empty arrays
        empty_hand = np.zeros((0, 3), dtype=kp2ds.dtype)
        empty_face = np.zeros((0, 3), dtype=kp2ds.dtype)
        return kp2ds_body, empty_hand, empty_hand, empty_face
    empty_hand = np.zeros((0, 3), dtype=kp2ds.dtype)
    return kp2ds_body, empty_hand, empty_hand


def load_pose_metas_from_kp2ds_seq(kp2ds_seq, width, height):
    """Load pose metadata from a sequence of animal keypoint detections.
    
    Args:
        kp2ds_seq: (N, 17, 3) array of animal keypoints [x, y, confidence]
        width: image width
        height: image height
    
    Returns:
        list of meta dicts with normalized keypoints_body (17, 3)
    """
    metas = []
    ...
    for kps in kp2ds_seq:
        ...
        kps[:, 0] /= width
        kps[:, 1] /= height
        kp2ds_body = split_kp2ds_for_animal(kps)
