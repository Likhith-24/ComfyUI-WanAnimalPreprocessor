...
def get_head_bboxes(kp2ds, scale, image_shape):
    """Get bounding box around animal head keypoints (eyes + nose).

    Args:
        kp2ds: Keypoints array of shape (17, 3) in pixel coords [x, y, conf]
        scale: Scale factor for expanding the bbox
    ...
    """
    h, w = image_shape
    # AP10k: L_Eye(0), R_Eye(1), Nose(2)
    head_kps = kp2ds[:3, :2].copy()

    min_x, min_y = np.min(head_kps, axis=0)
    max_x, max_y = np.max(head_kps, axis=0)

    initial_width = max(max_x - min_x, 1)
    initial_height = max(max_y - min_y, 1)
    ...
    expanded_min_y = max(min_y - delta_height, 0)
